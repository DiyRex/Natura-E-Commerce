...
public class ProductDAOImpl implements ProductDAO {
    ...
    @Override
    public List<Product> getAllProductsByKey(String searchKey) throws Exception {
        List<Product> products = new ArrayList<>();
        String sql = "SELECT p.Product_ID, p.Title, p.Description, p.Price, p.Qty, i.Image_Path "
                + "FROM product p LEFT JOIN image i ON p.Product_ID = i.Product_ID WHERE p.Title REGEXP ?";
        Connection conn = null;
        try {
            conn = DBConnection.getConnection();
            if (conn == null || conn.isClosed()) {
